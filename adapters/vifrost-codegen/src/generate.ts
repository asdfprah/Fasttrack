...
export function generateModelFile(
  ...
  const relationMethods = Object.entries(modelSchema.relations).map(([relationName, relation]) =>
    generateRelationMethod(relationName, relation, modelFqcn, generatedModelFqcns)
  )

  const relatedClassNamesToImport = Array.from(
    new Set(
      relationMethods
        .map((relationMethod) => relationMethod.importName)
  ...
  const relationsBlock = relationMethods.map((relationMethod) => relationMethod.code).join('\n\n')
